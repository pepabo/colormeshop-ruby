...
module ColorMeShop
  class GetSale200ResponseSaleAllOfSegment
    # 分割された受注ID
    attr_accessor :siblings_sale_ids

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'siblings_sale_ids' => :'siblings_sale_ids'
      }
    end
    ...
    def self.openapi_types
      {
        :'siblings_sale_ids' => :'Array<Integer>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ColorMeShop::GetSale200ResponseSaleAllOfSegment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ColorMeShop::GetSale200ResponseSaleAllOfSegment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'siblings_sale_ids')
        if (value = attributes[:'siblings_sale_ids']).is_a?(Array)
          self.siblings_sale_ids = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          siblings_sale_ids == o.siblings_sale_ids
    end
