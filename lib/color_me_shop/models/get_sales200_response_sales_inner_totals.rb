...
module ColorMeShop
  class GetSales200ResponseSalesInnerTotals
    # 受注における通常税率(10%)の消費税
    attr_accessor :normal_tax_amount

    # 受注における軽減税率(8%)の消費税
    attr_accessor :reduced_tax_amount

    # 通常税率(10%)の適用金額を対象としたの割引額
    attr_accessor :discount_amount_for_normal_tax

    # 軽減税率(8%)の適用金額を対象としたの割引額
    attr_accessor :discount_amount_for_reduced_tax

    # 通常税率(10%)の適用金額の税込合計額
    attr_accessor :total_price_with_normal_tax

    # 軽減税率(8%)の適用金額の税込合計額
    attr_accessor :total_price_with_reduced_tax
    ...
    def self.openapi_types
      {
        :'normal_tax_amount' => :'Integer',
        :'reduced_tax_amount' => :'Integer',
        :'discount_amount_for_normal_tax' => :'Integer',
        :'discount_amount_for_reduced_tax' => :'Integer',
        :'total_price_with_normal_tax' => :'Integer',
        :'total_price_with_reduced_tax' => :'Integer'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ColorMeShop::GetSales200ResponseSalesInnerTotals` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ColorMeShop::GetSales200ResponseSalesInnerTotals`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'normal_tax_amount')
        self.normal_tax_amount = attributes[:'normal_tax_amount']
      end

      if attributes.key?(:'reduced_tax_amount')
        self.reduced_tax_amount = attributes[:'reduced_tax_amount']
      end

      if attributes.key?(:'discount_amount_for_normal_tax')
        self.discount_amount_for_normal_tax = attributes[:'discount_amount_for_normal_tax']
      end

      if attributes.key?(:'discount_amount_for_reduced_tax')
        self.discount_amount_for_reduced_tax = attributes[:'discount_amount_for_reduced_tax']
      end

      if attributes.key?(:'total_price_with_normal_tax')
        self.total_price_with_normal_tax = attributes[:'total_price_with_normal_tax']
      end

      if attributes.key?(:'total_price_with_reduced_tax')
        self.total_price_with_reduced_tax = attributes[:'total_price_with_reduced_tax']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          normal_tax_amount == o.normal_tax_amount &&
          reduced_tax_amount == o.reduced_tax_amount &&
          discount_amount_for_normal_tax == o.discount_amount_for_normal_tax &&
          discount_amount_for_reduced_tax == o.discount_amount_for_reduced_tax &&
          total_price_with_normal_tax == o.total_price_with_normal_tax &&
          total_price_with_reduced_tax == o.total_price_with_reduced_tax
    end
    ...
    def hash
      [normal_tax_amount, reduced_tax_amount, discount_amount_for_normal_tax, discount_amount_for_reduced_tax, total_price_with_normal_tax, total_price_with_reduced_tax].hash
    end
