...
module ColorMeShop
  class UpdateSaleRequestSale
    # 入金済みであるか否か
    attr_accessor :paid

    # ショップポイントの付与状態。`fixed`へ変更すると、購入者にショップポイントが付与される。`fixed`から別のステータスには変更できない また、受注が分割されている場合、子受注のショップポイント付与状態は変更することができません。  - `assumed`: 仮付与 - `fixed`: 確定済み - `cenceled`: キャンセル済み 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ColorMeShop::UpdateSaleRequestSale` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ColorMeShop::UpdateSaleRequestSale`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      point_state_validator = EnumAttributeValidator.new('String', ["assumed", "fixed", "cenceled"])
    ...
    def point_state=(point_state)
      validator = EnumAttributeValidator.new('String', ["assumed", "fixed", "cenceled"])
    ...
    def hash
      [paid, point_state, sale_deliveries].hash
    end
