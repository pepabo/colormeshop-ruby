...
module ColorMeShop
  class StatSale200Response
    attr_accessor :sales_stat
    ...
    def self.openapi_types
      {
        :'sales_stat' => :'StatSale200ResponseSalesStat'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ColorMeShop::StatSale200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ColorMeShop::StatSale200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'sales_stat')
        self.sales_stat = attributes[:'sales_stat']
      end
    end
