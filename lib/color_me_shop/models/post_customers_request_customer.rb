...
module ColorMeShop
  class PostCustomersRequestCustomer
    # 購入者氏名
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ColorMeShop::PostCustomersRequestCustomer` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ColorMeShop::PostCustomersRequestCustomer`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'add_member')
        self.add_member = attributes[:'add_member']
      end
    end
    ...
    def hash
      [name, mail, pref_id, address1, address2, tel, postal, add_member].hash
    end
