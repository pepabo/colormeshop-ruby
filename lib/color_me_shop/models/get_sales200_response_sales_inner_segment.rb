...
module ColorMeShop
  class GetSales200ResponseSalesInnerSegment
    # 分割された受注内のID
    attr_accessor :id
    ...
    # 親受注のID。 該当の受注が分割されていない場合、自身の受注IDを返します。 
    attr_accessor :parent_sale_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ColorMeShop::GetSales200ResponseSalesInnerSegment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ColorMeShop::GetSales200ResponseSalesInnerSegment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'parent_sale_id')
        self.parent_sale_id = attributes[:'parent_sale_id']
      end

      if attributes.key?(:'splitted')
        self.splitted = attributes[:'splitted']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          id == o.id &&
          name == o.name &&
          parent_sale_id == o.parent_sale_id &&
          splitted == o.splitted &&
    ...
    def hash
      [id, name, parent_sale_id, splitted, product_total_price, delivery_total_charge, total_price, noshi_total_charge, card_total_charge, wrapping_total_charge].hash
    end
