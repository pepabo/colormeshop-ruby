  ...
  class PutProductPickupRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ColorMeShop::PutProductPickupRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ColorMeShop::PutProductPickupRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@order_num.nil? && @order_num < 0
        invalid_properties.push('invalid value for "order_num", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      pickup_type_validator = EnumAttributeValidator.new('Integer', [0, 1, 3, 4])
      return false unless pickup_type_validator.valid?(@pickup_type)
      return false if !@order_num.nil? && @order_num < 0
      true
    end
    ...
    def pickup_type=(pickup_type)
      validator = EnumAttributeValidator.new('Integer', [0, 1, 3, 4])
    ...
    # @param [Object] order_num Value to be assigned
    def order_num=(order_num)
      if order_num.nil?
        fail ArgumentError, 'order_num cannot be nil'
      end

      if order_num < 0
        fail ArgumentError, 'invalid value for "order_num", must be greater than or equal to 0.'
      end
